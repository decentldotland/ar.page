...
export function SearchBar(props: SearchType) {
  ...
  return (
    <div className={`px-4 flex border-gray-200 border-2 flex-row space-x-3.5 max-w-[80vw] md:max-w-none py-3 items-center 
    ${isDark ? ('bg-[#121a2f]'): ('')}
    rounded-2xl`}>
      ...
      <input
        ...
        className={`${isDark ? ('bg-[#121a2f]'): ('bg-inherit')}
          ...
          ${props.slideOutable ? "w-60 focus:w-[80vw] md:focus:w-60" : "w-full md:w-60"}
        `} />
    </div>
  )
}
