...
export const Nfts = (props: Props) => {
    ...
    return (
        <div className={props.className}>
            <div className="max-h-fit w-full mx-auto text-sviolet font-extrabold underline pt-1">
                <h1 className="text-sviolet text-lg font-extrabold underline text-center">NFTs: </h1>
            </div>
            ...
            {(NFTS && NFTS.length <= 0) ? <h1 className="lg:text-xl text-2xl text-center font-extrabold text-prim2 underline overflow-visible"></h1> :
                NFTS.map((owned: { title: string; poster: string; description: string; timestamp: number; id: string; }) =>
                <div key={owned.id} className={hasTwtr ? "lg:basis-1/2 lg:px-6 basis-full my-auto" : "lg:basis-1/3 lg:px-6 basis-full my-auto"} >
                    <div key={owned.id} className="flex flex-col shrink content-center my-4 py-4 px-3 text-center rounded-xl shadow-md border-2 border-prim1 shadow-gray-700">
                        <h1 className="lg:text-lg text-2xl text-center font-extrabold text-prim2 underline mb-2">{owned.title}</h1>
                        {/* <h1 className="text-xl mx-auto w-full text-white">Image: {owned.poster}</h1> */}
                        <div className="w-48 lg:w-24 lg:h-24 mx-auto">
                        ...
                        <h1 className="text-xl mx-auto font-bold text-white flex mt-2">{`Acquired `}<h1 className="text-lg mx-auto font-normal text-white">{`: ${moment.unix(owned.timestamp).format('DD/MM/YYYY')}`}</h1></h1>
                    </div>
                    </div>
                )}
            </div>
