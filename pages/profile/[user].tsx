...
const User = ({ userInfo }: any) => {
    ...
    return ( !userInfo.error && Object.keys(userInfo).length > 0 ?
        <div className="lg:grid grid-cols-3 grid-rows-2 gap-x-6 lg:max-h-screen">
             {hasTwtr === 2 ? <Personal userInfo={userInfo} className={(`my-6 p-6 mx-1 col-span-3 lg:col-span-2 lg:row-span-1 lg:py-6 rounded-3xl shadow-md border-2 border-prim1 shadow-gray-700 -m-6`)} />  :
            ...
            {hasTwtr !== 2 ? <Tweets user={userInfo.links.twitter} className="my-6 px-6 col-span-3 row-span-3 w-full overflow-y-hidden lg:hidden grid 0" /> : <></>}
